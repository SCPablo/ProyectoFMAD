...
h9 <- ggplot(datos)+
  geom_histogram(aes(x = suma_ofertas ), color = "black", alpha = 0.35)
h10 <- ggplot(datos)+
...
gridExtra::grid.arrange(h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11,h12)
...
\newpage
En esta sección usaremos técnicas de Machine Learning tomando distintos outputs. En primera instancia, el output será la variable 'num_hijos ' y 'marital_status' que las convertiremos en una variables binarias.
```{r}
datos_ML<-datos %>%
  mutate(sonPadres = factor(totalHijos>0, levels = c(TRUE, FALSE), labels = c(1,0)))
datos_ML<-datos_ML %>%
  mutate(Marital_Status = factor(Marital_Status== 'Single', levels = c(TRUE, FALSE), labels = c(1,0)))
```
...
```{r}
ggplot(datos_ML) + 
  geom_boxplot(aes(x = sonPadres, y = NumWebVisitsMonth))
```

```{r}
ggplot(datos_ML) + 
  geom_boxplot(aes(x = sonPadres, y = suma_ofertas))
```

# TODO Realizar algún modelo predictivo sobre variables target  
