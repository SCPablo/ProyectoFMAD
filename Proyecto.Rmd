...
datos %>%
  rowwise(ID) %>%
  mutate(suma_ofertas = sum(c(NumWebPurchases, NumStorePurchases, Response)))
```
...
datos %>%
  rowwise(ID) %>%
  mutate(respuestas = sum(c(AcceptedCmp3:AcceptedCmp2, Response))) %>%
  filter(respuestas == 2)
```
...
Contamos cuantos niños tiene el cliente en general
```{r}
datos %>%
  mutate(Niños = Kidhome + Teenhome)
```
...
\newpage
En esta sección usaremos técnicas de Machine Learning tomando distintos outputs. En primera instancia, el output serña la variable 'complain' la convertiremos en una variable binaria
```{r}
datos_ML<-datos %>%
