...
\newpage
En esta sección usaremos técnicas de Machine Learning tomando distintos outputs. En primera instancia, el output será la variable 'num_hijos 'que la convertiremos en una variable binaria.
```{r}
datos_ML<-datos %>%
  mutate(sonPadres = factor(totalHijos>0, levels = c(TRUE, FALSE), labels = c(1,0)))
borrar <- c("MntWines","MntSweetProducts","NumWebVisitsMonth","suma_ofertas", "sonPadres")
